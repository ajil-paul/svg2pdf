...
/// Represents a pre-encoded image XObject provided by an external source.
///
/// When an [`image_provider`](ConversionOptions::image_provider) callback
/// returns this, svg2pdf will skip its internal image decoding/encoding and
/// instead emit a coordinate transform followed by a `Do` operator that
/// references the given XObject name in the content stream.
pub struct ExternalImage {
    /// The XObject name to use in the content stream (e.g. `b"Im0"`).
    ///
    /// The caller is responsible for ensuring this name maps to a valid
    /// XObject in the relevant resource dictionary.
    pub name: Vec<u8>,
    /// The indirect reference to the pre-encoded image XObject.
    ///
    /// This reference will be added to the Form XObject's (or page's)
    /// resource dictionary so that the `Do` operator can resolve it.
    pub r#ref: Ref,
...
pub struct ConversionOptions {
    ...
    ///
    /// When set, svg2pdf will call this callback for every image node in the
    /// SVG tree. If the callback returns `Some(`[`ExternalImage`]`)`, svg2pdf
    /// will skip all image decoding/encoding for that node and instead emit
    /// the correct coordinate transform followed by a `Do` operator
    /// referencing the returned XObject name — preserving the image's
    /// z-order position in the content stream.
    ///
    /// If the callback returns `None`, the image is encoded into the chunk
    /// as usual (the default behavior).
    ///
    /// The [`ExternalImage::r#ref`] is added to the Form XObject's resource
    /// dictionary so that the `Do` operator can resolve it. The caller is
    /// responsible for ensuring the referenced XObject exists in the final
    /// PDF (e.g. by including it in the page's or document's chunks).
    ///
    /// _Default:_ `None`.
    pub image_provider: Option<Box<dyn Fn(&usvg::Image) -> Option<ExternalImage>>>,
}
